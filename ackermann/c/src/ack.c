...
void usage(void) {
  ...
  fprintf(stderr, "  -m | -i\tChooses the Ackermann implementation to use. -m "
                  "is memoized and -i is iterative. Specifying multiple "
                  "options will use the last specified.\n");
  fprintf(stderr, "  -l LOOPS\tRuns the algorithm with the given inputs LOOPS "
                  "times and analyzes the results.\n\n");
  fprintf(stderr, "  -s STACK_SIZE\tSets the max stack size in MB to "
                  "STACK_SIZE. Must be greater than 0.\n\n");
  fprintf(stderr, "Note that 'm' values greater than 4 are ill-advised- "
                  "ack(4,2) has 19 thousand digits.\n");
  exit(EXIT_FAILURE);
}
